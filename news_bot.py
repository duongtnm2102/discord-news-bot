...
# 🔧 FIXED: Enhanced User Agents with Latest Browser Versions - YAHOO FINANCE FIX
USER_AGENTS = [
    ...
    # 🔧 YAHOO FINANCE FIX: Added specific User-Agents that work with Yahoo Finance
    'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36',
    'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/108.0.0.0 Safari/537.36',
    'curl/7.68.0'  # 🔧 YAHOO FINANCE FIX: Added curl user-agent that works well with Yahoo Finance
]
...
# 🔧 FIXED: Enhanced RSS FEEDS with Multiple Yahoo Finance URLs - YAHOO FINANCE FIX
RSS_FEEDS = {
    ...
    # 🔧 YAHOO FINANCE FIX: Enhanced Yahoo Finance with Multiple URLs and Fallbacks
    'international': {
...
# 🔧 YAHOO FINANCE FIX: Enhanced headers specifically for Yahoo Finance
def get_yahoo_finance_headers(url=None):
    """Enhanced headers specifically for Yahoo Finance to avoid 403/429 errors"""
    ...
    # 🔧 YAHOO FINANCE FIX: Add specific headers for Yahoo Finance
...
async def search_yahoo_finance_by_title(title: str, max_results: int = 5):
    ...
    try:
        ...
        # 🔧 YAHOO FINANCE FIX: Updated Yahoo Finance RSS URLs with proper headers
        yahoo_rss_urls = [
        ...
        for rss_url in yahoo_rss_urls:
            try:
                ...
                session = requests.Session()
                # 🔧 YAHOO FINANCE FIX: Use Yahoo Finance specific headers
                headers = get_yahoo_finance_headers(rss_url)
                ...
                print(f"⚠️ Yahoo Finance RSS error for {rss_url}: {e}")
                continue
        ...
        print(f"⚠️ Yahoo Finance search error: {e}")
        return []
...
async def extract_yahoo_finance_content_enhanced(url: str):
    ...
    try:
        ...
        session = requests.Session()
        # 🔧 YAHOO FINANCE FIX: Use Yahoo Finance specific headers
        headers = get_yahoo_finance_headers(url)
        session.headers.update(headers)
...
# 🔧 YAHOO FINANCE FIX: Enhanced RSS Collection with Yahoo Finance specific handling
async def collect_news_stealth_enhanced(sources_dict, limit_per_source=6):
    ...
    for source_name, rss_url in sources_dict.items():
        try:
            ...
            # 🔧 YAHOO FINANCE FIX: Use Yahoo Finance specific headers for Yahoo sources
            if 'yahoo' in source_name.lower() or 'yahoo' in rss_url.lower():
                headers = get_yahoo_finance_headers(rss_url)
                print(f"🔧 Using Yahoo Finance headers for {source_name}")
            else:
            ...
            # 🔧 YAHOO FINANCE FIX: Enhanced error handling for Yahoo Finance
            try:
                response = session.get(rss_url, timeout=15, allow_redirects=True)
                print(f"📊 {source_name} response: {response.status_code}")
                
                if response.status_code == 403:
                    print(f"⚠️ 403 Forbidden for {source_name}, trying alternative headers...")
                    # Try with different user agent
                    alternative_headers = get_yahoo_finance_headers(rss_url)
                    alternative_headers['User-Agent'] = 'curl/7.68.0'  # Try curl user-agent
                    session.headers.update(alternative_headers)
                    
                    time.sleep(random.uniform(2.0, 4.0))
                    response = session.get(rss_url, timeout=15, allow_redirects=True)
                    print(f"🔄 Retry {source_name} response: {response.status_code}")
                
                if response.status_code == 429:
                    print(f"⚠️ Rate limited for {source_name}, waiting...")
                    time.sleep(random.uniform(5.0, 10.0))
                    response = session.get(rss_url, timeout=15, allow_redirects=True)
                    print(f"🔄 After wait {source_name} response: {response.status_code}")
                
                if response.status_code == 200:
                    feed = feedparser.parse(response.content)
                    print(f"✅ {source_name} parsed: {len(feed.entries) if hasattr(feed, 'entries') else 0} entries")
                else:
                    print(f"⚠️ {source_name} failed with status {response.status_code}, trying feedparser direct...")
                    feed = feedparser.parse(rss_url)
                    print(f"🔄 Direct parse {source_name}: {len(feed.entries) if hasattr(feed, 'entries') else 0} entries")
            ...
            print(f"✅ Successfully processed {entries_processed} entries from {source_name}")
            
        except Exception as e:
            print(f"❌ Critical error for {source_name}: {e}")
            continue
    ...
    print(f"✅ After deduplication: {len(unique_news)} unique news items")
    return unique_news
...
async def get_all_news_enhanced(ctx, page=1):
    ...
    try:
        ...
        emoji_map = {
            ...
            'yahoo_finance_main': '💰', 'yahoo_finance_news': '💰', 'yahoo_finance_business': '💼', 'yahoo_finance_markets': '📈'
        }
        ...
        source_names = {
            ...
            'yahoo_finance_main': 'Yahoo Finance', 'yahoo_finance_news': 'Yahoo Finance News', 
            'yahoo_finance_business': 'Yahoo Finance Business', 'yahoo_finance_markets': 'Yahoo Finance Markets'
        }
...
async def get_international_news_enhanced(ctx, page=1):
    ...
    try:
        ...
        emoji_map = {
            'yahoo_finance_main': '💰', 'yahoo_finance_news': '📰', 
            'yahoo_finance_business': '💼', 'yahoo_finance_markets': '📈'
        }
...
if __name__ == "__main__":
    try:
        ...
        # 🔧 YAHOO FINANCE FIX: Enhanced Yahoo Finance status check
        print("🔧 YAHOO FINANCE FIX STATUS:")
        print("   ✅ Enhanced User-Agent headers")
        print("   ✅ Multiple RSS feed URLs")
        print("   ✅ Enhanced error handling (403/429)")
        print("   ✅ Rate limiting and retry logic")
        print("   ✅ Curl user-agent fallback")
        print()
        ...
        print(f"   🌍 International sources: {len(RSS_FEEDS['international'])} (Yahoo Finance feeds)")
        print(f"   📊 Total sources: {total_sources}")
        print()
        
        # Yahoo Finance specific URLs
        print("🔧 Yahoo Finance RSS URLs (FIXED):")
        for source_name, url in RSS_FEEDS['international'].items():
        ...
        print("✅ Yahoo Finance RSS issue FIXED!")
        print()
